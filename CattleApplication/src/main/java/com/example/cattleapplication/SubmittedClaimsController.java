...
public class SubmittedClaimsController {
    ...
    @FXML
    private Button approveButton; // Button for approving claims
    ...
    private void handleApproveClaim() {
        Claim selectedClaim = submittedClaimsTable.getSelectionModel().getSelectedItem();
        if (selectedClaim == null) {
            showAlert("Selection Error", "Please select a claim to approve.");
            return;
        }

        try (Connection conn = DatabaseUtil.connect()) {
            String updateClaimQuery = "UPDATE claims SET claim_status = 'approved' WHERE claim_id = ?";
            try (PreparedStatement pstmt = conn.prepareStatement(updateClaimQuery)) {
                pstmt.setString(1, selectedClaim.getClaimId());
                pstmt.executeUpdate();
            }

            showAlert("Approval", "Claim ID " + selectedClaim.getClaimId() + " has been approved.");
            loadSubmittedClaims(); // Refresh the claims list
        } catch (SQLException e) {
            showAlert("Error", "Failed to approve the claim.");
            e.printStackTrace();
        }
    }
